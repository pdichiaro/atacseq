# pdichiaro/atacseq: Output

## Directory Structure by Normalization Method

This document shows the complete output directory structure for the ATAC-seq pipeline with different normalization methods.
...
pipeline_info/
├── nf_core_atacseq_software_mqc_versions.yml
...
- `invariant_genes` (default): Normalization using stable genes only (more robust for ATAC-seq)
...
- **Default mode**: `--nomodel` (no fragment length estimation)
  ...
  - Recommended for most ATAC-seq experiments
